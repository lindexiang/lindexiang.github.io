...
对于有序性，violatile和synchonorized都能保证有序性，violatile是防止指令重排序，synchonorized是由一个变量在同一时刻只能一个线程对其lock操作的这个规则获得的==。则决定了同一个锁的两个同步快只能串行的进入。**ReentrantLock是使用CAS来保证同步性，只能保证原子性，不能保证可见性和有序性。**
